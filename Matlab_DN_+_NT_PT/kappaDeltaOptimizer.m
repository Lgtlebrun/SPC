...
while ((abs(deltaTarget - delta)> tol) || (abs(kappaTarget - kappa) > tol))
    
    iterations = iterations+1;
    str = "Iteration number %d\n";
    fprintf(str, iterations);
...
COp = optimizeKappa(kappaTarget, deltaTarget, c0, x0, y0, c1, x1, y1, ...
    x2, y2, c3, x3, y3, x4, y4, I0, xv, yv,s);
...
disp("cv reached after optimization : ");
disp(cv);
disp("cx reached after optimization : ");
disp(cx);

[delta ,kappa, Rmin, Rmax] = computeSpecs(c0, c1, c2, c3, c4, x0, y0, x1, y1, ...
    x2, y2, x3, y3, x4, y4, I0, xv, yv,s);

disp("delta reached after elongation optimization : ");
disp(delta);
...
function COp = optimizeKappa(kappaTarg, deltaTarg, c0, x0, y0, c1, x1, y1, ...
    x2, y2, c3, x3, y3, x4, y4, I0, xv, yv,s)
...
contour(X,Y,Psi,'r','LevelList',Psi(iYxptlow,iXxpt));
